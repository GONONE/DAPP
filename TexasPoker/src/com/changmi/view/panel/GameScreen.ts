...
module game {
    ...
    export class GameScreen extends eui.Component {
        ...
        public createCompleteEvent() {
            ...
            // if (UserUtils.getInstance().getOwnUser().chairId != 0) {
                this.giveUpBtn.visible = false;
                this.addChipBtn.visible = false;
            // }
        ...
        public showPassBtn(): void {
            this.giveUpBtn.visible = true;
            this.addChipBtn.visible = true;
         ...
         */
        public sendPublicCard(round: number, multiple: boolean) {
            var cards = CardUtils.getInstance().getPublicCards();
            console.log("==================cards.length: ", cards.length);

            if (multiple) {
                let from: number = round > 0 ? round + 2 : 0;
         ...
         * 停止倒计时动画
         */
        public stopTimeRotate() {
            for (let i = 0; i < this.users.length; i++) {
                this.users[i].stoprotate();
            }
        }
        ...
        //通用加注效果
        public addChipAnimation(chip: number, userPosition: number, uId: string) {
            AnimationUtils.getInstance().addAnimation(function () {
                if (chip == 0) {
                    return;
                }
                ...
                // 更新玩家余额
                UserUtils.getInstance().getUserFromUid(uId).money -= chip;
        ...
        public giveChipAction(chip: number, userPosition: number) {
            ...
            console.log("----------------chipArray.length: ", chipArray.length);
