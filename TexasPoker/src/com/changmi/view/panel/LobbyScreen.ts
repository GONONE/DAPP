...
module game {
    export class LobbyScreen extends eui.Component {
        ...
        public resetUser() {
            ...
            if (UserUtils.getInstance().getOwnUser().pokerWallet == undefined) {
                this.text_gold_count.text = "0";
            } else {
                this.text_gold_count.text = "" + UserUtils.getInstance().getOwnUser().pokerWallet;
            }
        }
         ...
         */
        public showAddress() {
            let text: eui.TextInput = new eui.TextInput();
            text.textColor = 0x000000; // 设置颜色 
            text.width = 450;
            text.verticalCenter = 0;
            text.text = Wallet.getInstance().address[0];
